...
console.log(getContinentByCode('CN'));
...
function getContinentData(continentCode){
    ...
    let countries = countriesArr.map(({name}) => name)
    let area = countriesArr.reduce((accum, country) => {
        accum += country.area;
        return accum;
    },0);

    // { area: 10_324_532, countries: ['China', 'India'] }
